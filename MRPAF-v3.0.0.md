# Multi-Resolution Pixel Art Format (MRPAF) Specification v3.0.0
...
This document is the finalized `v3.0.0` specification text.

The English text is the normative reference text for `v3.0.0`. Any translated edition is informative unless stated otherwise.
...
Implementations should not assume compatibility with pre-v3 MRPAF experimental drafts unless compatibility is stated explicitly in this document.
...
```json
{
  ...
  "$id": "https://mrpaf.org/schemas/v3.0.0/mrpaf.schema.json",
  "title": "MRPAF v3.0.0 core schema",
  "type": "object",
...
This release intentionally narrows MRPAF to an editable interchange core.

Compared with earlier internal drafts and broader exploratory designs, this release:

- fixes the top-level core object model
- removes alternative coordinate-system machinery
- removes serialized derived size fields
- removes runtime-heavy animation control from the core format
- limits core pixel encodings to `array`, `rle`, and `sparse`
- treats unknown extensions as ignorable but preservable data
- centers AI-to-editor handoff and iterative revision instead of final delivery
