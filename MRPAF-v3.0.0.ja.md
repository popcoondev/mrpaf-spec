# Multi-Resolution Pixel Art Format (MRPAF) 仕様書 v3.0.0
...
この文書は、`v3.0.0` の正式仕様書である。

英語版を `v3.0.0` の規範的な正本とする。翻訳版は、別途明記しない限り参考訳である。

今後の保守や改訂では、次の点に注意すること。
...
本書に明記されていない限り、実装は v3 より前の MRPAF 実験ドラフトとの互換性を前提にしてはならない。
...
以下の schema は、この仕様書に整合する最小構造コア schema である。
...
このリリースは、MRPAF を編集可能な交換コアへ意図的に絞り込んでいる。

以前の内部ドラフトや、より広い探索的設計と比べて、このリリースは次を行っている。

- トップレベルの core object model を固定した
- 代替座標系の仕組みを削除した
- 導出サイズフィールドのシリアライズを削除した
- ランタイム依存の強いアニメーション制御をコア形式から除外した
- core pixel encoding を `array`、`rle`、`sparse` に限定した
- 未知の extension を無視可能だが保持可能なデータとして扱う
- 最終配布ではなく AI-to-editor handoff と iterative revision を中心に据えた

## 保守チェックリスト

この仕様の改訂版を公開する前に、次のチェックリストを使うこと。
