# Multi-Resolution Pixel Art Format (MRPAF) 仕様書 v3.1.0
...
この文書は、`v3.1.0` の正式仕様書である。

英語版を `v3.1.0` の規範的な正本とする。翻訳版は、別途明記しない限り参考訳である。
...
本書に明記されていない限り、実装は v3 より前の MRPAF ドラフトとの互換性を前提にしてはならない。
...
このリリースは、コア描画意味を変えずに標準の layer-group 拡張を追加する。

`v3.0.0` と比べて、このリリースは次を行っている。
